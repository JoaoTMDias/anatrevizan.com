...
export const EditorialCollection: Collection = {
	...
	fields: [
		...
		{ name: 'routeKey', label: 'Canonical route', type: 'string', required: true, options: routeKeys.map((value) => ({ label: value, value })) },
		...localizedDocumentFields.slice(2),
		{ name: 'body', label: 'Page content', type: 'string', ui: { component: 'textarea' } },
		{ name: 'ctaRefs', label: 'CTA identifiers', type: 'string', list: true },
		{ name: 'legal', label: 'Legal document controls', type: 'object', fields: [{ name: 'version', label: 'Reviewed version', type: 'string' }, { name: 'effectiveDate', label: 'Effective date', type: 'datetime' }, { name: 'reviewedBy', label: 'Qualified reviewer', type: 'string' }] },
	],
};
