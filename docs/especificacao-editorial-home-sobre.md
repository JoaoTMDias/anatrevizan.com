...
- Na importação, todo o texto entrou em `draft`, com `approvalPending: true` e `noindex: true`. Importação não equivale a aprovação factual, profissional, académica, jurídica ou linguística.
- Em 24 de agosto de 2026, a Ana reviu e aprovou integralmente o conteúdo PT-PT da Home e Sobre no TinaCMS. `approvalPending` passou a `false`; ambas permanecem em `draft` e `noindex` porque os respetivos destinos ainda não foram migrados. O conteúdo EN continua por traduzir e aprovar.

## Auditoria das 19 páginas

| Grupo | Rotas canónicas | Fonte Next.js | Estado na auditoria |
|---|---|---|---|
| 1 | Home; Sobre | 33 + 57 strings de página, interface partilhada e três destaques ORCID | Migrado neste grupo; PT aprovado em 24-08-2026, ainda draft/noindex; EN é cópia integral de PT e não é tradução válida |
...
|---|---|---|
| Home `messages/pt-pt/home.json` | Hero, 5 palavras de marca, 2 portas, 4 diferenciais, títulos de Consultoria e Academia, 5 credenciais e CTA final | Migrado integralmente para campos estruturados TinaCMS; PT aprovado, mantido draft/noindex por dependências |
...
- A Ana aprovou em 24 de agosto de 2026 as afirmações PT-PT sobre OAB, graus académicos, bolsas, vínculos, docência, certificação, idiomas, jurisdições e experiência. A tradução EN e qualquer revisão especializada adicional permanecem pendentes.
